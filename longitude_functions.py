...
def delta_longitude_calculation(x_N, y_N, z_N, 
    ...
    t_TEB_CS = numpy.sum(delta_length_mfl_B_S_CS / v_A_S[:-1]) + length_mfl_B_S_out_CS / c  + (length_mfl_B_S + length_mfl_B_N)/ v


    t_MAW_CS = numpy.sum(delta_length_mfl_B_N_CS / v_A_N[:-1]) + length_mfl_B_N_out_CS / c
...
def objective_function_zlimN_from_longitude(z_lim_N, r_0, rho_0_volumetric_first_guess,
    ...
    mask_CS_N = (z_N <= z_lim_N) & (equatorial_radius_N > 10) # Second condition is to be sure no points close to the planet are taken
    ...
    longitude_MAW, longitude_TEB = longitude_MAW_TEB_calculation(x_N, y_N, z_N, B_N,
                                                                x_S, y_S, z_S, B_S,
                                                                mask_CS_N, mask_CS_S,
    ...
    if TEB:
        ...
        #print(f'{(longitude - longitude_observed) ** 2:.02f}')
